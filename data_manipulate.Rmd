...
ggplot(month_rank,aes(x=month,y=n,group=type,color=type))+
  geom_line()

chen_poster=chen%>%group_by(poster=chen$artPoster)%>%count()%>%arrange(desc(n))
han_poster=han%>%group_by(poster=han$artPoster)%>%count()%>%arrange(desc(n))
